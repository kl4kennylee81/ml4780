...
x1_pos_vector = posprob.*x1;
x1_pos_no_zeros = x1_pos_vector(x1_pos_vector>0);
x1_pos_log = log(x1_pos_no_zeros);
x1_pos_PXY = sum(x1_pos_log,1);
poscond =  x1_pos_PXY + log(py_pos);



x1_neg_vector = negprob.*x1;
x1_neg_no_zeros = x1_neg_vector(x1_neg_vector>0);
x1_neg_log = log(x1_neg_no_zeros);
